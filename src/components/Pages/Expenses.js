...
const Expense = (props) => {
  ...
  const totalExpense = arrayDetails.reduce(
    (accumulator, curValue) => parseInt(curValue.amount) + accumulator,
    0
  );
  ...
  useEffect(() => {
    let initialData = [];
    fetch(
      `https://expense-tracker-3135a-default-rtdb.firebaseio.com/${loggedEmail}.json`
    )
      .then((res) => {
        if (res.ok) {
          return res.json();
      ...
      .then((data) => {
        for (let [key, val] of Object.entries(data)) {
          ...
          setArrayDetails((arrayDetails) => [
            ...arrayDetails,
            {
              name: key,
              amount: val.Amount,
              description: val.Description,
              expenses: val.Category,
            },
          ]);
        }
      });
  ...
  const updateHandler = (item) => {
    ...
    let NewDescriptioin = prompt(
      " Enter the New Desription: ",
      item.description
    );
    ...
    console.log(item);
    fetch(
      `https://expense-tracker-3135a-default-rtdb.firebaseio.com/${loggedEmail}/${item.name}.json`,
      {
        method: "GET",
      }
    )
    ...
    function reqPATCH(id, NewAmount, NewCategory, NewDescription) {
      fetch(
        `https://expense-tracker-3135a-default-rtdb.firebaseio.com/${loggedEmail}/${id}.json`,
        {
          method: "PATCH",
          body: JSON.stringify({
            amount: NewAmount,
  ...
  const deleteHandler = (id) => {
    // console.log(id.name)
    fetch(
      `https://expense-tracker-3135a-default-rtdb.firebaseio.com/${loggedEmail}/${id.name}.json`,
      {
        method: "DELETE",
      }
    )
    ...
    <div>
      <form className="form" onSubmit={submitHandler}>
        ...
        <span className="form-span">
          <label className="form-lable" htmlFor="amount">
            Amount Spent Rs :
          </label>
          <input
            className="form-input-AE"
            type="number"
            min="0"
            id="amount"
            ref={amountRef}
        ...
        <span className="form-span">
          ...
          >
            ...
            <option className="form-option" value="Movie">
              Movie
            </option>
            <option className="form-option" value="Petrol">
              Petrol
            </option>
            <option className="form-option" value="Shopping">
              Shopping
            </option>
            <option className="form-option" value="Personal">
              Personal
            </option>
        ...
        <span>
          ...
          <button className="submit-expense" onClick={crtctx.removeToken}>
            Logout
          </button>
          {showPremium && (
            <button
          ...
          {showPremium && (
            <button
              ...
              onClick={premiumFeatures}
            >
              <CSVLink {...csvReport}>Download_Expense</CSVLink>
            </button>
          )}
        </span>
        <br></br>
